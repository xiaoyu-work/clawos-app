...
pub use playback::{MprisEvent, MprisMeta, MprisState, PlaybackCommand};
...
impl App {
    ...
    fn update_mpris_meta(&mut self) {
        let mut new = MprisMeta {
            url_opt: self.video_opt.as_ref().and(self.flags.url_opt.clone()),
    ...
    fn update(&mut self, message: Self::Message) -> Task<Self::Message> {
        match message {
            ...
            Message::Stop => {
                ...
                if let Some(video) = &mut self.video_opt {
                    video.set_paused(true);
                    match video.seek(Duration::ZERO, true) {
                        Ok(()) => self.position = 0.0,
                        Err(error) => {
                            log::warn!("failed to reset stopped playback: {error}");
            ...
            Message::PlayPrev => {
                ...
                if let Some(video) = &mut self.video_opt {
                    ...
                    if self.position > 3.0 {
                        ...
                        if curr_position.checked_sub(1).is_some_and(|position| self.nav_model.activate_position(position)) {
                            let curr_id = self.nav_model.active();
            ...
            Message::PlayNext => {
                ...
                if curr_position.checked_add(1).is_some_and(|position| self.nav_model.activate_position(position)) {
                    let curr_id = self.nav_model.active();
                    match self.nav_model.data::<ProjectNode>(curr_id) {
            ...
            Message::MprisCommand(command) => {
                return self.update(command.into());
            }
            Message::NewFrame => {
                if let Some(video) = &mut self.video_opt
                    && !self.dragging
...
impl From<PlaybackCommand> for Message {
    fn from(command: PlaybackCommand) -> Self {
        match command {
            PlaybackCommand::Play => Self::Play,
            PlaybackCommand::Pause => Self::Pause,
            PlaybackCommand::Stop => Self::Stop,
            PlaybackCommand::Toggle => Self::PlayPause,
            PlaybackCommand::Next => Self::PlayNext,
            PlaybackCommand::Previous => Self::PlayPrev,
            PlaybackCommand::Repeat(state) => Self::RepeatToggled(state),
            PlaybackCommand::Volume(volume) => Self::AudioVolume(volume),
        }
    }
}
