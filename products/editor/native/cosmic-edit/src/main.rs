...
async fn run_ai_doc(
    ...
    tokio::task::spawn_blocking(move || {
        claw_glue::ai::transform_text(text, verb.operation(), instruction)
    })
    .await
...
pub enum AiVerb {
    ...
    Explain,
    Rewrite,
    /// Free-form rewrite instruction through the same SDK AI gate.
    Custom,
}
    ...
    fn update(&mut self, message: Message) -> Task<Message> {
        ...
        match message {
            ...
            Message::NewWindow => {
                ...
                if let Err(err) = crate::claw_glue::new_window() {
                    log::error!("failed to open editor window: {}", err);
                }
            }
