...
xdg-entry-applications-keywords = ClawOS;Default;Application;Startup;X11;Compatibility
...
xdg-entry-date-time-keywords = ClawOS;Time;Zone;
...
xdg-entry-default-apps-keywords = ClawOS;Default;Application
...
xdg-entry-x11-applications-keywords = ClawOS;X11;Application;Game;Compatibility;
...
settings = Settings
ai-search-no-results = No settings page matches “{ $query }”.
ai-search-ask-claw = Ask Claw to find this setting
ai-search-ask-claw-hint = Press to open the Claw AI overlay and let it search for you.
share = Share network
username = Username
...
enable-export = Apply current theme to GNOME apps
    .desc = Not all toolkits support auto-switching. Non-ClawOS apps may need to be restarted after a theme change.
...
app-permissions-description = Manage declared brokered permissions for your Apps. Settings never inherits their grants or approves restoration. Human confirmation belongs to OS review; direct-resource and argument-bound permissions cannot yet be changed here.
...
app-permissions-pending = Restoration request submitted for OS review. No execution grant was created; refresh to check owner policy.
...
app-permissions-review-in-os = These requests remain pending until reviewed in the OS approval gate or with cos review, not in Settings. Refresh to see the OS decision. Cancelling OS authentication leaves the request pending.
