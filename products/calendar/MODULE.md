...
| `native/build.py`, `native/Cargo.lock`, `../../tools/native_build.py`, `../../tools/native-patches.toml` | Shared native build/test against the locked OS toolkit |
...
inputs under `build/calendar-native`, uses the shared `build/native-target` cache,
and compiles with the checked-in lock.
