...
| OS `core/src/clawd/packages.rs` | `require_app("pkg")` is additional to `sys.package` scope checks. Retain protection until an equivalent generic capability/resource-owner check and regressions replace the business-name gate. |
| OS app-permissions and media-player routes | Business-name gates for `cosmic-settings` and `cosmic-player` remain instead of a generic granted-capability/resource-owner contract. Keep their current defenses until replacements are validated. |
