...
| App-service persistent storage | The OS binds the existing owner/App data partition through an exact-App private idmapped view. Host/UID replacement does not copy, chown or delete its backing files. Legacy task-host operation binding and Calendar's cross-App read service remain incomplete; this does not complete installed-App or GUI resource acceptance. |
