...
def test_messaging_channels_stage_nested_connector_without_shared_runtime_or_state(tmp_path):
    assert stage.stage("messaging-channels", tmp_path) == ["gateway-discord", "gateway-dingtalk", "gateway-googlechat", "gateway-larksuite", "gateway-matrix", "gateway-mattermost", "gateway-rocketchat", "gateway-signal"]
    for channel in ("discord", "dingtalk", "googlechat", "larksuite", "matrix", "mattermost", "rocketchat", "signal"):
